...
func RedisCreateUser(u tools.UserRequest) error {
	// 1. Conversion Struct -> Map (comme pour Mongo)
	doc, err := tools.ToMap(u)
	if err != nil {
...
func RedisCreateSession(s tools.SessionsRequest) error {
	doc, err := tools.ToMap(s)
	if err != nil {
...
func RedisLoadUser(ID int, Username string, Email string, Phone string) (tools.UserRequest, error) {
	var u tools.UserRequest
	...
	if err := tools.ToStruct(docs[0], &u); err != nil {
...
func RedisLoadSession(ID int, DeviceToken string) (tools.SessionsRequest, error) {
	var s tools.SessionsRequest
	...
	if err := tools.ToStruct(docs[0], &s); err != nil {
