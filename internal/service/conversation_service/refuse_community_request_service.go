...
func RefuseCommunityRequest(ctx context.Context, callerID int64, input conversation_models.RefuseCommunityRequestInput) (conversation_models.RefuseCommunityRequestOutput, error) {
	...
	if err != nil {
		return conversation_models.RefuseCommunityRequestOutput{}, nubo_error.NewForbidden("ACCESS_DENIED", "Conversation introuvable ou accès refusé.", err)
	}
	if callerMem.Role < 1 {
		return conversation_models.RefuseCommunityRequestOutput{}, nubo_error.NewForbidden("INSUFFICIENT_PERMISSIONS", "Vous devez être administrateur pour refuser une candidature.", nil)
	}
	...
	if err != nil || targetMem.ID == 0 {
		...
		if err != nil || targetMem.ID == 0 {
			...
			if err != nil || targetMem.ID == 0 {
				return conversation_models.RefuseCommunityRequestOutput{}, nubo_error.NewNotFound("USER_NOT_FOUND", "Candidature introuvable.", err)
			}
	...
	if targetMem.Role != -3 {
		return conversation_models.RefuseCommunityRequestOutput{}, nubo_error.NewBadRequest("INVALID_STATE", "Cet utilisateur n'est pas en attente d'approbation.", nil)
	}
	...
	output := conversation_models.RefuseCommunityRequestOutput{}
