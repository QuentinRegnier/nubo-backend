...
func DemoteMember(ctx context.Context, callerID int64, input conversation_models.DemoteMemberInput) (conversation_models.DemoteMemberOutput, error) {
	...
	if err != nil {
		return conversation_models.DemoteMemberOutput{}, nubo_error.NewForbidden("ACCESS_DENIED", "Conversation introuvable ou accès refusé.", err)
	}
	if callerMem.Role != 2 {
		return conversation_models.DemoteMemberOutput{}, nubo_error.NewForbidden("INSUFFICIENT_PERMISSIONS", "Seul le propriétaire peut destituer un administrateur.", nil)
	}
	...
	if err != nil || targetMem.Role < 0 {
		return conversation_models.DemoteMemberOutput{}, nubo_error.NewBadRequest("USER_NOT_MEMBER", "L'utilisateur ciblé n'est pas membre de ce groupe.", err)
	}

	// 3. RÈGLES MÉTIER ET IDEMPOTENCE
	if targetMem.Role == 0 {
		return conversation_models.DemoteMemberOutput{}, nil // Déjà membre normal, on valide silencieusement
	}
	if targetMem.Role == 2 {
		return conversation_models.DemoteMemberOutput{}, nubo_error.NewForbidden("CANNOT_DEMOTE_OWNER", "Impossible de destituer le propriétaire, transférez d'abord la propriété.", nil)
	}
	...
	output := conversation_models.DemoteMemberOutput{}
