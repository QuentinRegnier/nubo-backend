...
func GetBannedMembers(ctx context.Context, callerID int64, input conversation_models.GetBannedMembersInput) (conversation_models.GetBannedMembersOutput, error) {
	...
	if err != nil || mem.Role < 1 {
		return conversation_models.GetBannedMembersOutput{}, nubo_error.NewForbidden("ACCESS_DENIED", "Seuls les administrateurs peuvent voir la liste des bannis.", nil)
	}
	...
	if err != nil {
		return conversation_models.GetBannedMembersOutput{}, nubo_error.NewInternal(err)
	}

	var bannedUsers []conversation_models.BannedUserView
	...
	for _, bMem := range bannedMembers {
		if userLite, errLite := cache_service.GetUserLite(ctx, bMem.UserID); errLite == nil {
			...
			bannedUsers = append(bannedUsers, conversation_models.BannedUserView{
	...
	if bannedUsers == nil {
		bannedUsers = make([]conversation_models.BannedUserView, 0)
	}

	return conversation_models.GetBannedMembersOutput{BannedUsers: bannedUsers}, nil
}
