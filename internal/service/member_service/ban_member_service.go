...
func BanMember(ctx context.Context, callerID int64, input member_models.BanMemberInput) (member_models.BanMemberOutput, error) {
	...
	if err != nil {
		return member_models.BanMemberOutput{}, nubo_error.NewForbidden("ACCESS_DENIED", "Conversation introuvable ou accès refusé.", err)
	}
	if callerMem.Role < 1 { // Doit être au moins Admin (1)
		return member_models.BanMemberOutput{}, nubo_error.NewForbidden("INSUFFICIENT_PERMISSIONS", "Vous devez être administrateur ou propriétaire pour bannir un membre.", nil)
	}
	...
	if err != nil {
		...
		return member_models.BanMemberOutput{}, nubo_error.NewBadRequest("USER_NOT_MEMBER", "L'utilisateur ciblé n'est pas un membre actif de ce groupe.", err)
	}

	// 3. VÉRIFICATION HIERARCHIQUE
	if callerMem.Role <= targetMem.Role {
		return member_models.BanMemberOutput{}, nubo_error.NewForbidden("HIERARCHY_VIOLATION", "Vous ne pouvez pas bannir un membre de rang égal ou supérieur.", nil)
	}
	...
	if err == nil {
		go func() {
			...
			if err != nil {
				logger.Log.Error().Err(err).Msg("Failed to broadcast member ban")
			}
		}()
	}

	output := member_models.BanMemberOutput{}
