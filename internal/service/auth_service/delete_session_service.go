...
func RevokeSession(ctx context.Context, callerID int64, sessionID int64) error {
	...
	// 1. TENTATIVE L1 (RAM) : Récupération pour vérifier la propriété et obtenir le DeviceToken (pour l'index)
	...
	_ = cache_service.DeleteSessionFromCache(ctx, sessionID, s.UserID, s.DeviceToken)

	// 4. PERSISTANCE ASYNCHRONE : Envoi au Worker pour suppression SQL
