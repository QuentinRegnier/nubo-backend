...
// cache en mémoire dans le process Go"
// ─────────────────────────────────────────────────────────────────────────────

type simMatrixState struct {
	G         []float32 // Matrice G[n×n] aplatie row-major: G[i*n+j] = <ĉ_{p_i}, ĉ_{p_j}>
	n         int
	postIDs   []int64 // Liste ordonnée des post IDs utilisés pour construire G
	expiresAt time.Time
}
...
func ComputePersonalizedScore(
	...
	// ── B(u,p) — Indicateur de post d'un ami direct
	//
	// TDD §4.2: B(u,p) ∈ {0, 1}
...
// getOrBuildSimMatrix retourne la matrice de similarité depuis le cache ou la reconstruit.
//
// TDD §4.3:
//
//	"calculée une fois par fenêtre temporelle (toutes les 5 minutes) et mise en
...
// candidatesMatch vérifie si la liste de post IDs correspond aux candidats actuels.
...
//   - totalN: dimension de G (= len(candidates) pour matrice on-the-fly, ou |C| pour cache)
...
// InjectSerendipity remplace aléatoirement des slots du feed par des posts de découverte.
//
// TDD §4.3:
//
...
func InjectSerendipity(feed []PostCandidate, pool []int64, probPerSlot float64, rng *rand.Rand) []PostCandidate {
	...
	for i := range feed {
		...
		if rng.Float64() < probPerSlot {
			// Sélection uniforme dans le pool global (top-N du trend feed)
