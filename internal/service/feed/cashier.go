...
// BuildPersonalizedFeed construit le feed personnalisé de K posts pour un utilisateur à partir des candidats.
func BuildPersonalizedFeed(ctx context.Context, opts PersonalizedFeedOptions) ([]int64, error) {
	...
	if err != nil {
		log.Printf("⚠️ [personalized] erreur mise en buffer feed %d: %v", opts.UserID, err)
	}
