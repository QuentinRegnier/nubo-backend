...
// ScoredCandidate retient le score mathématique du post depuis le ZSET Redis.
...
func (pf *ProtoFeedBuilder) CollectCandidates(ctx context.Context, userID int64, quotas Quotas) (*CandidateBasket, error) {
	...
	// Clé du feed issue du Fan-Out on write
	...
	if err != nil {
		// FALLBACK : On ne bloque pas tout le feed si la liste sociale est inaccessible.
	...
	// On demande au cache les 5 tags préférés de l'utilisateur.
	...
	// Plus tau est proche de 0, plus on sélectionne le post avec le score maximum (ZRANGE classique).
	tauTags := 1.5
...
func (pf *ProtoFeedBuilder) fetchLongTailMongo(ctx context.Context, userID int64, basket *CandidateBasket, countNeeded int) error {
	...
	for countNeeded > 0 && maxAttempts > 0 {
		...
		for _, p := range posts {
			...
			// L'ID du post provient de ta structure domain.PostRequest
			postID := p.ID

			// Double sécurité : on valide quand même face au Cuckoo
			// (Au cas où le tirage aléatoire retombe sur un post très ancien mais déjà vu)
