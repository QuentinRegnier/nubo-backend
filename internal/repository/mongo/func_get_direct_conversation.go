...
func MongoGetDirectConversation(user1, user2 int64) (conversation_models.ConversationPayload, error) {
	if Members == nil || Conversations == nil {
		return conversation_models.ConversationPayload{}, fmt.Errorf("collections mongo non initialisées")
	}
	...
	defer func(cursor *mongo.Cursor, ctx context.Context) {
		...
		if err != nil {
			_ = fmt.Errorf("erreur fermeture cursor Mongo : %v", err)
		}
	}(cursor, ctx)

	var results []bson.M
	if err = cursor.All(ctx, &results); err != nil || len(results) == 0 {
		return conversation_models.ConversationPayload{}, fmt.Errorf("not found")
	}

	// CORRECTION : Assertion de type pour forcer l'interface{} en bson.M (map[string]any)
	convMap, ok := results[0]["conv"].(bson.M)
	if !ok {
		return conversation_models.ConversationPayload{}, fmt.Errorf("impossible de formater le résultat mongo en map")
	}
