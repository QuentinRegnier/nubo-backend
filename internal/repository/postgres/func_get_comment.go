...
func FuncGetComment(ctx context.Context, commentID int64) (comment_models.CommentPayload, error) {
	...
	query := `SELECT id, post_id, user_id, content, visibility, created_at, updated_at FROM content.func_get_comment($1)`
	err := postgres.PostgresDB.QueryRowContext(ctx, query, commentID).Scan(
		&c.ID, &c.PostID, &c.UserID, &c.Content, &c.Visibility, &c.CreatedAt, &c.UpdatedAt,
	)
