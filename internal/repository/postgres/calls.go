...
func FuncLoadUser(ID int, Username string, Email string, Phone string) (domain.UserRequest, error) {
	...
	if ID == -1 && Username == "" && Email == "" && Phone == "" {
		return domain.UserRequest{}, fmt.Errorf("erreur: champs requis manquants pour FuncLoadUser (ID %d)", functionID)
	}
	...
	if err != nil {
		return domain.UserRequest{}, fmt.Errorf("erreur lors de l'exécution de FuncLoadUser (ID %d): %w", functionID, err)
	}
...
func FuncLoadSession(ID int, DeviceToken string) (domain.SessionsRequest, error) {
	...
	err := postgres.PostgresDB.QueryRow(sqlStatement, args...).Scan(&res.ID, &res.UserID, &res.RefreshToken, &res.DeviceToken, &deviceInfoBytes, &res.IPHistory, &res.CreatedAt, &res.ExpiresAt)
	if err != nil {
