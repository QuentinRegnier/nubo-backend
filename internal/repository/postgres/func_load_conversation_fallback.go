...
func FuncLoadConversationFallback(ctx context.Context, userID int64, convIDs []int64) ([]InboxFallbackResult, error) {
	query := `SELECT conversation_id, title, type, last_message_id, role, unread_count, frozen_message_id, joined_at FROM messaging.func_load_conversation_fallback($1, $2)`
	...
	for rows.Next() {
		...
		if err := rows.Scan(&cid, &title, &cType, &lastMsgID, &role, &unreadCount, &frozenID, &joinedAt); err == nil {
