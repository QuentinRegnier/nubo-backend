...
import (
	...
	redis2 "github.com/QuentinRegnier/nubo-backend/internal/repository/redis"
...
func checkAndEvictDynamic(ctx context.Context, rdb *redis.Client, margin int64) {
	...
	for usedMemory > triggerLimit && safetyLoop < 50 {
		...
		for _, item := range items {
			...
			redis2.registryMu.RLock()
			coll, exists := redis2.collectionRegistry[collName]
			redis2.registryMu.RUnlock()
